...
import { fetchEventWizardBanner, updateEventWizardBanner } from "@/api/events"
...
export function EventBannerStepPage() {
  ...
  const bannerQuery = useQuery({
    queryKey: ["events", "wizard-draft", eventId, "banner"],
    queryFn: () => {
      if (!eventId) {
  ...
  const updateMutation = useMutation({
    mutationFn: (payload: { bannerFile: File | null; clearBanner: boolean }) => {
      ...
      return updateEventWizardBanner(eventId, payload, 4)
    },
    onSuccess: async (data) => {
      ...
      setValue("bannerUrl", data.bannerUrl ?? "", { shouldDirty: false, shouldTouch: false, shouldValidate: false })
      queryClient.setQueryData(["events", "wizard-draft", eventId, "banner"], data)
      queryClient.setQueryData(["events", "wizard-progress", eventId], (current: { stepNo?: number } | undefined) => ({
        stepNo: Math.max(current?.stepNo ?? 0, data.stepNo ?? 4),
      }))
  ...
  const stageBannerFile = useCallback(
    ...
    [revokeStagedPreview, setSelectedUnsplashPhoto],
  )
  ...
  useEffect(() => {
    if (bannerQuery.data) {
      setValue("bannerUrl", bannerQuery.data.bannerUrl ?? "", { shouldDirty: false, shouldTouch: false, shouldValidate: false })
    }
  }, [bannerQuery.data, setValue])
  ...
  useEffect(() => {
    ...
    setPrimaryActionEnabled(Boolean(previewBannerUrl))
        ...
        <Flex align="flex-start" justify="space-between" gap={4} wrap="wrap">
          <Box minW={0}>
            ...
            <Text mt={2} fontSize={{ base: "sm", md: "md" }} fontWeight="700" color="gray.800">
              Add a banner image for this event. This step is required, so there is no Skip action.
            </Text>
            <Text mt={2} fontSize="sm" color="gray.600">
              JPG, PNG, and WEBP images work best for cards, headers, and review.
            </Text>
          </Box>
          ...
          >
            <Flex align="flex-start" justify="space-between" gap={3} wrap="wrap">
              <Box>
                ...
                <Text mt={1} fontSize="xs" fontWeight="700" color="gray.500">
                  {bannerStateLabel}
                </Text>
                <Text mt={1} fontSize="sm" color="gray.600" lineHeight="1.6">
                  {previewBannerUrl
                    ? "This is the banner that will be used when you continue."
                    : "You cannot continue without selecting a banner image."}
                </Text>
              </Box>
              ...
              {previewBannerUrl ? (
                ...
                <Stack gap={2} align="center" textAlign="center" px={6}>
                  ...
                  <Text fontSize="sm" color="gray.600" lineHeight="1.6" maxW="sm">
                    Pick an image from the left before saving.
                  </Text>
                </Stack>
              )}
            </Box>
            ...
            <Flex align="center" justify="space-between" gap={3} wrap="wrap">
              <Text fontSize="sm" color="gray.600">
                {previewBannerUrl ? "Replace or remove the image before saving." : "Choose an upload or Unsplash image to continue."}
              </Text>
